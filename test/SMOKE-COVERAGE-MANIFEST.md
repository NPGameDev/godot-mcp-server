...
| editor_get_console | 14 | ✓ | ✓ (INVALID_PARAMS) | ✓ (level_filter, text_filter plain+regex, since_id, source=buffer/file) | — | **GAP:** clear_buffer param. ledger #9: total_lines/next_id/truncated. **Editor parse-error capture is 4.5+ only** (Logger); 4.2-4.4 don't file-log editor parse errors → §14 gates the parse-error-filter assertions (#2/#3/#6) to 4.5+. "at:" continuation leveling for captured multi-line errors is toolkit-side + unit-tested (41m-ter A2/A3). **Headless:** those capture assertions also self-skip (`&& !headless`); §14 positively asserts the deterministic `headless_hint` (steers to script_check) whenever error capture is requested. **`source=file`:** on 4.3+ the CI composite passes `--log-file user://logs/godot.log` (globalizes to the reader's `user://logs/` dir — source-verified) so §14 requires the real file-read path (entries on Linux; a Windows-local run sees LOG_BUSY = file present), keyed on the `SMOKE_EXPECT_FILE_LOG` harness signal; 4.2 (no `--log-file` flag) + any no-`--log-file` run assert `LOG_UNAVAILABLE` + (headless) the `headless_hint` steering to `source="buffer"` — 41n-quater-bis |
