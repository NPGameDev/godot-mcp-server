...
| runtime_get_node_state | 17 | ✓ (also **paused-tree responsiveness**: returns node state while `get_tree()` is paused — the runtime autoload's `PROCESS_MODE_ALWAYS` keeps it servicing WS calls when the game loop is suspended) | — | — | — | In runtime_advanced group. §17 self-provides a runtime (reuse a live game or launch `current` + poll 6570) and green-skips the happy paths where none is launchable (headless / no editor) — deterministic, never asserts GAME_NOT_RUNNING as a "pass". **Pause-immunity leg (try/finally):** `execute.code get_tree().set_pause(true)` → assert `get_tree().paused === true` (anti-vacuous guard) → assert `runtime.get_node_state /root` still RETURNS (the regression — before the autoload ran `PROCESS_MODE_ALWAYS` this froze while paused); `finally` ALWAYS unpauses via `set_pause(false)` (a leaked pause would suspend the runtime for every later section) |
