...
| Tool Name | Smoke Section | Happy Path | Guard Tests | Param Variations | Hint Assertions | Notes |
|---|---|---|---|---|---|---|
| runtime_screenshot | 01, 17 | ✓ | ✓ (GAME_NOT_RUNNING; 17 manual-assist: RUNTIME_WINDOW_MINIMIZED) | ✓ (**force_foreground_game** — §01 asserts the param on inputSchema) | ✓ (17 manual-assist: `remediation` foregrounded_game) | §01 (structural, headless/CI): advertises `force_foreground_game` + `RUNTIME_WINDOW_MINIMIZED` in ErrorCode union. §17 minimized legs (minimized→code, force_foreground recover + `remediation:["foregrounded_game"]`) are **manual-assist** — gated on `MCP_MANUAL_ASSIST=1` (green-skip unattended; positive coverage in toolkit sweep §20) |
