...
export async function testSignalsAndIntrospection(ctx: TestCtx): Promise<void> {
  ...
  const signalProbeNode = (await bridge.call(
    "scene.create_node",
    { class_name: "Node", parent_path: ".", node_name: "SignalProbe" },
    CALL_TIMEOUT,
  ...
  const signalListResult = (await bridge.call("signal.list", { node_path: signalProbePath }, CALL_TIMEOUT)) as {
    signals?: { name?: string; args?: unknown[] }[];
    code?: string;
  };
  if (!Array.isArray(signalListResult?.signals) || signalListResult.signals.length === 0)
    fail(`signal.list: ${JSON.stringify(signalListResult)}`);
  else if (!signalListResult.signals.some((s) => s.name === "child_order_changed"))
  ...
  const connectFresh = (await bridge.call("signal.manage", { action: "connect", ...connectionArgs }, CALL_TIMEOUT)) as {
    status?: string;
    code?: string;
    signal?: string;
  };
  ...
  const connectIdempotent = (await bridge.call(
    "signal.manage",
    { action: "connect", ...connectionArgs },
    CALL_TIMEOUT,
  )) as { status?: string; code?: string };
  if (connectIdempotent?.status !== "returned")
    fail(`signal.manage connect idempotency: expected status='returned', got ${JSON.stringify(connectIdempotent)}`);
  else if (connectIdempotent.code !== undefined)
    fail(`signal.manage connect collision success must not carry code (got ${connectIdempotent.code})`);
  else pass("signal.manage connect repeat -> status='returned' + code absent");

  const emitResult = (await bridge.call(
    "signal.emit",
  ...
  if (disconnectRepeat?.code !== "NOT_FOUND")
    fail(`signal.manage disconnect repeat: expected NOT_FOUND, got ${JSON.stringify(disconnectRepeat)}`);
  else pass("signal.manage disconnect + NOT_FOUND on repeat");

  // node.get_property_list.
  const propertyList = (await bridge.call("node.get_property_list", { node_path: signalProbePath }, CALL_TIMEOUT)) as {
    properties?: { name?: string; type?: number; hint?: number; hint_string?: string }[];
    count?: number;
    code?: string;
  };
  if (!Array.isArray(propertyList?.properties) || typeof propertyList.count !== "number") {
    ...
    if (!names.has("process_mode"))
      fail(`node.get_property_list: expected process_mode, got ${Array.from(names).slice(0, 5).join(",")}...`);
  ...
  const loadedResource = (await bridge.call("resource.load", { file_path: "res://icon.svg" }, CALL_TIMEOUT)) as {
    class?: string;
    path?: string;
  ...
  else if (!loadedResource.metadata?.width || !loadedResource.metadata.height)
    fail(`resource.load icon.svg: missing width/height in metadata: ${JSON.stringify(loadedResource.metadata)}`);
  else
    pass(
      `resource.load icon.svg -> class=${loadedResource.class} ${loadedResource.metadata.width}x${loadedResource.metadata.height}`,
    );

  const missingResource = (await bridge.call(
    "resource.load",
    { file_path: "res://does_not_exist_smoke.tres" },
    CALL_TIMEOUT,
  )) as { code?: string };
  if (missingResource?.code !== "NOT_FOUND")
    fail(`resource.load bogus: expected NOT_FOUND, got ${JSON.stringify(missingResource)}`);
