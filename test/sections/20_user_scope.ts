...
export async function testUserScope(ctx: TestCtx): Promise<void> {
  ...
  if (process.env.MCP_ENABLE_USER_SCOPE !== "1") {
    ...
    if (!featureEnabled("read_user_scope")) {
      pass("[skip] user-scope smoke requires MCP_ENABLE_USER_SCOPE=1 AND Godot launched with GODOT_MCP_ALLOW_USER_SCOPE=1 + mcp/unsafe/allow_user_scope=true");
    } else {
  ...
  const readResult = await bridge.call("save.read", {
    path: "user://saves/smoke.json",
  ...
  await bridge.call("save.write", {
    path: "user://saves/smoke_big.json",
    content: bigContent,
  }, CALL_TIMEOUT);
  const truncRead = await bridge.call("save.read", {
    path: "user://saves/smoke_big.json",
    max_bytes: 1024,
  }, CALL_TIMEOUT) as { success?: boolean; truncated?: boolean; total_bytes?: number; bytes_returned?: number };
  if (truncRead?.success !== true || truncRead.truncated !== true) {
    ...
    pass(`save.read truncation -> truncated=true, bytes_returned=${truncRead.bytes_returned}, total_bytes=${truncRead.total_bytes}`);
  }

  // save.read oversized max_bytes.
  assertGuard(ctx, "save.read oversized max_bytes",
    await bridge.call("save.read", { path: "user://saves/smoke.json", max_bytes: 500000 }, CALL_TIMEOUT),
    "INVALID_PARAMS", "262144");

  // save.list happy path.
  const listResult = await bridge.call("save.list", {
    path: "user://saves/",
  }, CALL_TIMEOUT) as { success?: boolean; files?: string[]; file_count?: number };
  ...
  const delResult = await bridge.call("save.delete", {
    path: "user://saves/smoke.json",
  }, CALL_TIMEOUT) as { success?: boolean };
  ...
  assertError(ctx, "save.delete NOT_FOUND",
