...
export async function testInputMap(ctx: TestCtx): Promise<void> {
  ...
  const addActionResult = (await bridge.call(
    "input_map.action",
    { action: "add", action_name: smokeAction, deadzone: 0.4 },
    CALL_TIMEOUT,
  ...
  if (addActionResult?.status === "returned") {
    try {
      await bridge.call("input_map.action", { action: "remove", action_name: smokeAction }, CALL_TIMEOUT);
    } catch {
      /* noop */
    }
  ...
  const addActionIdempotent = (await bridge.call(
    "input_map.action",
    { action: "add", action_name: smokeAction, deadzone: 0.9 },
    CALL_TIMEOUT,
  )) as { status?: string; deadzone?: number; code?: string };
  if (
  ...
  const addKeyEvent = (await bridge.call(
    "input_map.event",
    { action: "bind", action_name: smokeAction, event: { type: "key", keycode: "SPACE" } },
    CALL_TIMEOUT,
  ...
  const addJoypadEvent = (await bridge.call(
    "input_map.event",
    { action: "bind", action_name: smokeAction, event: { type: "joypad_button", button_index: 0, device: -1 } },
    CALL_TIMEOUT,
  )) as { status?: string; code?: string };
  ...
  const removeKeyEvent = (await bridge.call(
    "input_map.event",
    { action: "unbind", action_name: smokeAction, event: { type: "key", keycode: "SPACE" } },
    CALL_TIMEOUT,
  ...
  assertGuard(
    ...
    await bridge.call("input_map.action", { action: "remove", action_name: "ui_accept" }, CALL_TIMEOUT),
    "INVALID_PARAMS",
    ["built-in UI action"],
  );
  assertGuard(
    ...
    await bridge.call(
      "input_map.event",
      { action: "bind", action_name: smokeAction, event: { type: "telepathy" } },
      CALL_TIMEOUT,
    ),
    "INVALID_PARAMS",
  ...
  assertGuard(
    ...
    await bridge.call(
      "input_map.event",
      { action: "bind", action_name: smokeAction, event: { type: "key", keycode: "NONSENSE" } },
      CALL_TIMEOUT,
    ),
    "INVALID_PARAMS",
    "symbolic names",
  ...
  assertGuard(
    ...
    await bridge.call("input_map.action", { action: "add", action_name: "" }, CALL_TIMEOUT),
    "INVALID_PARAMS",
    "non-empty",
  );
