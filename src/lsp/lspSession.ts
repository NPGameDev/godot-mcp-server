...
import type { ToolTextResult } from "../shared/types.js";
import { toolError } from "../shared/errorContract.js";
import { LspClient, LspResolutionError, type LspStatus } from "./lspClient.js";
...
let lspClient: LspClient | undefined = undefined;

function getLspClient(projectPath: string): LspClient {
  if (!lspClient) lspClient = new LspClient(projectPath);
  return lspClient;
}
...
async function ensureLsp(projectPath: string): Promise<ToolTextResult | LspClient> {
  ...
  try {
    ...
    const ep = client.getEndpoint();
    statusReporter?.({ state: "unavailable", host: ep.host, port: ep.port, detail: (err as Error).message });
    return toolError(
      "LSP_UNAVAILABLE",
