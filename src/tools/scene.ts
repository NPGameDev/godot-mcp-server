...
export const sceneTools: ToolDef[] = [
  ...
  },
  {
    ...
    method: "scene.create",
    description:
      "Create .tscn at file_path. Root name = filename stem at '.'. root_type default Node. Idempotent: created|returned|replaced. if_exists: return|fail|replace (replace clears all nodes and reloads if open). Use scene_open afterward to edit.",
    inputSchema: {
      file_path: z.string(),
