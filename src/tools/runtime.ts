...
export const runtimeTools: ToolDef[] = [
  ...
  },
  {
    ...
    description:
      ...
      "Types: key|mouse_button|mouse_motion|action|click|click_node|send_text. click is a composite: press + 50ms delay + release via push_input (no OS focus/warp; parallel-safe). " +
      ...
      "Mouse events route through push_input (position + global_position) for CanvasLayer/GUI hit-testing — no OS focus/warp, parallel-safe. Returns per-event diagnostics.",
    inputSchema: {
      events: z.union([
        z.object({
